...
public class SortPost {
	String filename_in;
	String filename_out;
	int M; // size of chunk to sort in number of lines, also the size of a
			// sorted sublist
	int B; // size of buffer in characters (1 char = 2 bytes), if B==8192 char,
			// it is 16K bytes
	int c; // sort column
	...
	public void doSort() throws Exception {
		...
		while (true) {
			...
			if ((sCurrentLine = readers[minh.i].readLine()) != null) {
				heads.add(new HeadIndexPair(sCurrentLine, minh.i));
			}
		}
