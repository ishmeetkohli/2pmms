...
import bean.TaxPayer;
import tools.TaxPayerComparator;
...
public class SortPost {
	...
	public void doSort() throws Exception {
		...
		while ((sCurrentLine = in.readLine()) != null) {
			if(cnt < M) {
				chunk[cnt] = sCurrentLine;
				cnt++;	
			} else {
				numChunks++;
				cnt = 0;
				ArrayList<TaxPayer> taxPayerList = getObjectListFromChunk(chunk, sep);
				Collections.sort(taxPayerList, new TaxPayerComparator());	
				// TODO
//				Write these to files
	...
	private ArrayList<TaxPayer> getObjectListFromChunk(String[] chunk, String separator) {
		ArrayList<TaxPayer> taxPayerList = new ArrayList<TaxPayer>();
		
		for(String record : chunk) {
			String[] recordArray = record.split(separator);
			taxPayerList.add(new TaxPayer(recordArray[0], recordArray[1], recordArray[2], Integer.parseInt(recordArray[3])));
		}
